...
# ─── GitHub MCP Server (optional, configurable per project) ─────────────────
# Only written if .mcp.json doesn't already exist — never overwrites a
# project's existing MCP config. Defaults to github.com; GitHub Enterprise
# Server projects add a GITHUB_HOST entry themselves (see SKILL.md / README).
...
MCPEOF
  ...
  echo "  ✓ Created .mcp.json (github.com by default — export GITHUB_PERSONAL_ACCESS_TOKEN; see .skills/github-issues/SKILL.md for GitHub Enterprise Server setup)"
  echo ""
elif ! grep -q '"github"' "$MCP_FILE" 2>/dev/null; then
  echo "→ GitHub MCP server..."
  echo "  ⚠ .mcp.json already exists with no \"github\" entry — add one manually if you want /superspecs:issues to work (see .skills/github-issues/SKILL.md)"
  echo ""
fi
...
echo "    /superspecs:verify      (or /superspecs-verify)"
