...
summary: How flashcard-session models a multi-step practice flow (pick → speak → reveal → flag) as pure, immutable state transitions, kept separate from React and from the DB. Extended by session-size-summary with a configurable session size and flag-order tracking, by editorial-redesign with an auto-advance consumer pattern, by user-accounts with a per-learner Home/PracticeScreen split, by star-total with an independent header badge query, by star-progress-display with a goal-distance header message and a session-progress indicator, and by weighted-session-selection with a relative-score bias toward the learner's weakest words.
tags: [ui, flashcard-session, session-size-summary, editorial-redesign, user-accounts, star-total, star-progress-display, weighted-session-selection, state-management, react]
spec: "[[flashcard-session]]"
created: 2026-09-16
updated: 2026-09-25
provenance:
  sources: [specs/flashcard-session/spec.md, specs/flashcard-session/GRILL.md, phases/flashcard-session-execute/review-log.md, specs/session-size-summary/spec.md, specs/session-size-summary/GRILL.md, phases/session-size-summary-execute/review-log.md, specs/editorial-redesign/spec.md, phases/editorial-redesign-execute/review-log.md, specs/user-accounts/spec.md, phases/user-accounts-execute/review-log.md, specs/star-total/spec.md, phases/star-total-execute/review-log.md, specs/star-progress-display/DISCUSS.md, specs/star-progress-display/spec.md, specs/star-progress-display/GRILL.md, phases/star-progress-display-execute/review-log.md, specs/weighted-session-selection/DISCUSS.md, specs/weighted-session-selection/spec.md, specs/weighted-session-selection/GRILL.md, phases/weighted-session-selection-execute/review-log.md]
  extracted: 65%
  inferred: 30%
  ambiguous: 5%
...
### `startSession` biases toward the learner's relatively weakest words
**Chose:** weighted-session-selection (2026-09-25) replaced `startSession`'s uniform shuffle-and-slice with a weighted draw: a new exported pure helper, `splitByRelativeScore(words)`, sorts the bank by the learner's own score ascending and splits it at the halfway point (rounded up) into a `weak` pool (lower half) and a `rest` pool (upper half) — relative to the current bank and learner, not a fixed score threshold like `score <= 0`. `startSession` then draws `min(weak.length, ceil(sessionSize / 2))` words from `weak` first, fills the remaining slots from all not-yet-drawn words (leftover `weak` + all of `rest`), and shuffles the combined result before returning it. `startSession`'s external signature and contract (`(words, sessionSize) => Word[]`, same count/no-duplicate/whole-bank-when-smaller behavior) are unchanged.
**Over:** A fixed score threshold (e.g. `score <= 0`) to define "weak" — explicitly rejected mid-discussion: a fixed threshold doesn't scale with how practiced a learner is overall (a very advanced learner might have almost nothing at or below 0; a brand-new learner has everything at 0). Also rejected: strictly separating the remainder draw to the strong pool only (the remainder draws from *all* remaining words, so the weak proportion can exceed 50% but never falls below the guaranteed minimum), and shrinking the session size to preserve an exact 50/50 ratio when the weak pool can't fill its half (the session always stays full, up to `min(available words, sessionSize)`).
**Because:** the learning-effect goal is best served by a relative, self-adjusting definition of "weak" that always reflects roughly half the bank regardless of the learner's overall skill level, combined with a hard floor (not a ceiling) on how often struggling/never-practiced words appear. Reduced repetition of mastered words is an accepted natural side effect of this bias, not a separately engineered anti-repetition mechanism (no new "last practiced" tracking was added).
**Trade-off:** none significant for the real 533-word bank, where the weak pool (~266 words) is always far larger than any configured session's minimum draw (max 32 for a 64-word session). The only place the "weak pool can't fill the minimum" path is actually reachable is when the *whole bank* is smaller than the configured session size — mathematically equivalent to the pre-existing "whole pool used when bank is smaller than sessionSize" case, not a new failure mode.
**Testability decision (grilled):** the weak/rest split logic was deliberately pulled out into its own exported function (`splitByRelativeScore`) rather than left inline inside `startSession`, specifically so it can be asserted deterministically (exact pool membership/counts on a known input) instead of only being observable through repeated random draws of `startSession`'s full output. The randomized parts of the feature (the weak-pool minimum draw itself, and the final shuffle) are still necessarily tested via repeated-draw/statistical assertions (e.g. "at least 12 of 24 drawn words are in the weak pool, across 75 repeated draws") — worth remembering as the pattern for testing any future randomized-selection logic in this codebase: separate the deterministic decision logic from the randomized draw so each can be tested with the appropriate technique.
